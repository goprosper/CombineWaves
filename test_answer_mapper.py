...
class TestComputeAnswerIds:
    ...
    def test_basic_mapping(self):
        ...
        result = compute_answer_ids("3^2^?^1^6^7^4^5", 7)
    ...
    def test_sequential_mapping(self):
        ...
        result = compute_answer_ids("1^2^3", 3)
        assert result == "0^1^2"
    ...
    def test_reordered_mapping(self):
        ...
        result = compute_answer_ids("2^1", 2)
        assert result == "1^0"
    ...
    def test_large_answer_count(self):
        ...
        result = compute_answer_ids(answer_value_map, 12)
        assert result == "1^3^0^2^5^4^7^6^9^8^11^10"

    def test_single_answer(self):
        """Only one answer maps correctly."""
        result = compute_answer_ids("1", 1)
        assert result == "0"
    ...
    def test_map_with_non_numeric_values(self):
        ...
        result = compute_answer_ids("?^1^2", 2)
        assert result == "1^2"

    def test_partial_missing_answers(self):
        """Some answers found, some not found."""
        # Map has 1, 3 but not 2
        # For answer 1, find "1" -> index 0
        # For answer 2, find "2" -> not found -> empty string
        # For answer 3, find "3" -> index 1
        result = compute_answer_ids("1^3", 3)
        assert result == "0^^1"
    ...
    def test_none_answer_value_map(self):
        ...
        result = compute_answer_ids(None, 5)
        assert result == ""
